...
const outputLines = [];
const fakeChannel = {
    appendLine: (msg) => { outputLines.push(msg); process.stdout.write(`\x1b[2m  ${msg}\x1b[0m\n`); },
    append: (msg) => outputLines.push(msg),
    show: () => { },
    clear: () => { },
    dispose: () => { },
};
const vscodeMock = {
    workspace: {
        ...
        asRelativePath: (uri) => path.relative(WORKSPACE_ROOT, typeof uri === 'string' ? uri : uri.fsPath),
    },
    Uri: {
    ...
    window: {
        showErrorMessage: (m) => console.error('[vscode.error]', m),
        showWarningMessage: (m) => console.warn('[vscode.warn]', m),
...
Module._resolveFilename = (req, ...rest) => req === 'vscode' ? 'vscode' : _origResolve(req, ...rest);
...
───────────────────────────────────────────────────────────────────────────── */
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { WorkspaceIndexer } = require('./indexer');
// eslint-disable-next-line @typescript-eslint/no-require-imports
...
const info = (t) => console.log(`${C.blue}  ℹ ${t}${C.reset}`);
const warn = (t) => console.log(`${C.red}  ✖ ${t}${C.reset}`);
const dim2 = (t) => console.log(`${C.dim}${t}${C.reset}`);
/* ─────────────────────────────────────────────────────────────────────────────
   Graph callbacks — mirrors agent.ts logic without any vscode dependency
───────────────────────────────────────────────────────────────────────────── */
function makeCallbacks(indexer) {
    const root = indexer.getRoot() ?? WORKSPACE_ROOT;
    const expandSelections = (selections, prompt) => {
        const tokens = (prompt.match(/[A-Za-z_$][A-Za-z0-9_$]*/g) ?? [])
            .filter((t) => t.length >= 3);
        const known = new Set((indexer.index?.files ?? []).map((f) => f.relPath.replace(/\\/g, '/')));
        const traversed = indexer.guidedTraversal(selections.map(s => s.relPath), tokens, { direction: 'forward', maxDepth: 3, maxFiles: 8 });
    ...
    const discoverSecondPass = (fileContents, prompt) => {
        ...
        const q = fileContents.map(f => ({
            relPath: f.relPath, depth: 0,
        }));
        for (const f of fileContents) {
            for (const caller of indexer.getDependents(f.relPath)) {
                add(caller, 1);
            }
        }
        while (q.length > 0) {
        ...
        const words = new Set((prompt.match(/[A-Za-z_$][A-Za-z0-9_$]*/g) ?? []).filter((w) => w.length >= 3));
        for (const word of words) {
            ...
            if (ctx) {
                ...
                for (const rp of indexer.getFilesExportingSymbol(word)) {
                    add(rp, 5);
                }
            }
            for (const rp of indexer.getImportersOfSymbol(word)) {
                add(rp, 3);
            }
        }
        for (const rp of indexer.searchFiles([...[...words].map((w) => w.toLowerCase())])) {
            add(rp, 2);
        }
    ...
    const resolveFiles = async (selections) => {
        const known = new Set((indexer.index?.files ?? []).map((f) => f.relPath));
        const out = [];
...
/* ─────────────────────────────────────────────────────────────────────────────
   Single test run
───────────────────────────────────────────────────────────────────────────── */
async function runTest(indexer, prompt) {
    ...
    const result = await runPipeline(prompt, indexer.buildSelectionGraph(), [], {
        ...
        onStage(stage) {
            const now = Date.now();
            if (lastLabel) {
                timing.push({ label: lastLabel, ms: now - stageStart });
            }
            stageStart = now;
            lastLabel = stage;
...
async function main() {
    ...
    const t0 = Date.now();
    const index = await indexer.build((done, total) => {
    ...
    [...index.files]
        ...
        .slice(0, 10)
        .forEach((f, i) => dim2(`  ${String(i + 1).padStart(2)}.  score:${String(f.baseScore).padStart(3)}  ${f.relPath}`));
