...
const callGraphExtractor_1 = require("./callGraphExtractor");
...
class WorkspaceIndexer {
    constructor(_outputChannel, storageUri) {
        ...
        this._idf = new Map(); // keyword → idf score (recomputed after each full build/patch)
        this._symbolImporters = new Map(); // "sym@srcRelPath" → files that import sym from srcRelPath
        this._callGraph = null; // function call graph for context selection
    ...
    async tryLoad() {
        ...
        try {
            ...
            for (const f of data.files) {
                ...
                else if (Array.isArray(f.keywords)) {
                    f.keywords = Object.fromEntries(f.keywords.map(k => [k, 1]));
                }
                if (!f.keywordLines) {
                    f.keywordLines = {};
                }
    ...
    _processAndIndex(absPath, root) {
        ...
        const { keywords, keywordLines } = large ? { keywords: {}, keywordLines: {} } : (0, symbolExtractor_1.extractKeywords)(content, relPath);
     ...
     */
    refineSelectionsWithCallGraph(selections) {
        if (!this._callGraph) {
        ...
        for (const sel of selections) {
            ...
            const fnMatch = Array.from(this._callGraph.functions.values()).find(f => f.relPath === sel.relPath && f.lineStart <= (sel.lineStart ?? 0) && f.lineEnd >= (sel.lineEnd ?? 0));
            if (!fnMatch) {
            ...
            const context = (0, callGraphExtractor_1.getSemanticContext)(key, this._callGraph, 1);
            refined.push({
                relPath: sel.relPath,
    ...
    guidedTraversal(entryPaths, queryTokens, opts = {}) {
        ...
        for (const file of this._index?.files ?? []) {
            ...
            for (const sym of Object.keys(this._namedImports.get(key) ?? {})) {
                const symParts = new Set((0, symbolExtractor_1.splitIdentifier)(sym));
            ...
            for (const token of lowerTokens) {
                ...
                if (tf !== undefined) {
                    score += (tf * (this._idf.get(token) ?? 1)) / lenNorm;
                }
            }
            ...
            if (lowerTokens.length >= 2) {
                ...
                if (closePairs > 1) {
                    proximityScore += closePairs - 1;
                } // bonus for multiple tight pairs
                score += Math.min(proximityScore, 10);
            }
            // Hub boost
            if (file.baseScore) {
                score += Math.log1p(file.baseScore) * 0.5;
            }
        ...
        const visited = new Set(seeds.map(([k]) => k));
        const queue = seeds.map(([relPath, score]) => ({
    ...
    _buildCallGraph() {
        ...
        for (const file of this._index.files) {
            ...
            try {
                ...
                const functions = (0, callGraphExtractor_1.extractFunctionDefs)(content, file.relPath);
        ...
        this._callGraph = (0, callGraphExtractor_1.buildCallGraph)(allFunctions);
     ...
     */
    getSemanticContext(functionKey, depth = 1) {
        if (!this._callGraph) {
            return [];
        }
        return (0, callGraphExtractor_1.getSemanticContext)(functionKey, this._callGraph, depth);
    }
    /**
     * Export call graph as JSON for visualization/debugging.
     * Saves to: .vscode/call-graph.json
     */
    async exportCallGraphJson() {
        ...
        try {
            ...
            const graphData = {
                ...
                callsGraph: Object.fromEntries(Array.from(this._callGraph.callsGraph.entries()).map(([k, v]) => [k, Array.from(v)])),
                calledByGraph: Object.fromEntries(Array.from(this._callGraph.calledByGraph.entries()).map(([k, v]) => [k, Array.from(v)])),
                stats: {
