...
export interface FunctionDef {
  ...
  isAsync: boolean;
  isExported: boolean;
  calls: string[];              // function names this calls
  usesTypes: string[];          // type names used in signature
}

export interface CallGraphData {
  functions: Map<string, FunctionDef>;        // "file:function" → definition
  callsGraph: Map<string, Set<string>>;       // "file:func" → {"file:func", ...}
  calledByGraph: Map<string, Set<string>>;    // "file:func" → {"file:func", ...}
}
 ...
 */
export function extractFunctionDefs(content: string, relPath: string): FunctionDef[] {
  ...
  while ((match = fnRegex.exec(content)) !== null) {
    ...
    while (closeIdx < content.length && braceCount > 0) {
      if (content[closeIdx] === '{') braceCount++;
      else if (content[closeIdx] === '}') braceCount--;
      closeIdx++;
    }
 ...
 */
function extractTypesFromSignature(signature: string): string[] {
 ...
 */
export function buildCallGraph(allFunctions: FunctionDef[]): CallGraphData {
  const functions = new Map<string, FunctionDef>();
  const callsGraph = new Map<string, Set<string>>();
  const calledByGraph = new Map<string, Set<string>>();
...
export function getSemanticContext(
  ...
  const queue: [string, number][] = [[targetFunctionKey, 0]];

  while (queue.length > 0) {
    const [key, d] = queue.shift()!;
    if (context.has(key) || d > depth) continue;
    context.add(key);
