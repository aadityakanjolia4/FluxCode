...
export interface ReviewIssue {
  type: 'correctness' | 'consistency' | 'quality' | 'performance';
  file: string;
  message: string;
  suggestion: string;
}
...
export async function inferCodeStyle(
  apiKey: string,
  model: string,
  fileContents: { relPath: string; content: string }[]
): Promise<string> {
...
async function runFileSelector(
  ...
  const messages = [
    ...stampedHistory(history),
    { role: 'user' as const, content: `Workspace graph:\n\n${fileTree}\n\n---\nRequest: ${userPrompt}` },
  ];
  try {
...
export async function thinkAboutQuery(
  apiKey: string,
  model: string,
  history: Message[],
  prompt: string
...
- needsResearch: boolean (should I gather context first?)`,
  ...
  try {
    const result = await requestWithTool<ThinkingResult>(
      apiKey,
      model,
