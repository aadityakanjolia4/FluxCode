...
export function validateEdits(
  ...
  const valid: RawClaudeEdit[] = [];
  const skipped: Array<{ edit: RawClaudeEdit; reason: string }> = [];
...
export function validatePlanCoverage(plan: CodePlan, edits: RawClaudeEdit[]): string[] {
...
export function fuzzyFindReplace(current: string, oldStr: string, newStr: string): string | null {
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (parsed.error) { reject(new Error(`API: ${parsed.error.message}`)); return; }
...
async function requestWithTool<T>(
  apiKey: string,
  model: string,
  system: string,
  messages: { role: string; content: string }[],
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            if (toolUse?.input) { resolve(toolUse.input as T); }
            else {
...
export async function selectFiles(
  ...
  try {
    const parsed = extractJson(text) as { thinking?: string; filesToRead?: string[] };
...
function formatPlanBlock(plan: CodePlan): string {
  if (!plan.steps.length) { return '(no plan steps)'; }
...
function formatEditsBlock(edits: RawClaudeEdit[]): string {
  return edits.map((e) => {
    if (e.isNew) {
...
export async function generateEditsParallel(
  apiKey: string,
  model: string,
  history: Message[],
  userPrompt: string,
  ...
  const args = [apiKey, model, history, userPrompt, fileContents, plan] as const;
  const [r1, r2] = await Promise.allSettled([
    generateEdits(...args),
    generateEdits(...args),
  ]);

  if (r1.status === 'fulfilled' && r2.status === 'rejected') { return r1.value; }
  if (r1.status === 'rejected' && r2.status === 'fulfilled') { return r2.value; }
  if (r1.status === 'rejected' && r2.status === 'rejected') { throw r1.reason; }

  const v1 = validateEdits((r1 as PromiseFulfilledResult<RawTurnResult>).value.edits, fileContents);
  const v2 = validateEdits((r2 as PromiseFulfilledResult<RawTurnResult>).value.edits, fileContents);

  return v1.valid.length >= v2.valid.length
    ? (r1 as PromiseFulfilledResult<RawTurnResult>).value
    : (r2 as PromiseFulfilledResult<RawTurnResult>).value;
}
...
export async function reviewEdits(
  ...
  try {
    const result = await requestWithTool<{
      thinking?: string; approved?: boolean; feedback?: string; issues?: string[];
